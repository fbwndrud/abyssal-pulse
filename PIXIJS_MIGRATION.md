# 어비스 펄스 — PixiJS v8 마이그레이션 브리핑

> 작성: 2026-05-03
> 다음 세션에서 이 문서를 읽고 계획을 세움.
...
3. **뷰포트 크기**는 PixiJS 한계 외부 조사 기반으로 결정 — 권장: **1280×720** (16:9 표준, 화면 안 동시 sprite 한도 ~300, PixiJS 한계 5000보다 항상 낮은 안전 마진).
...
- `/Users/lyoojk/.claude/plans/bright-puzzling-mango.md` — 이전 계획 (융합 시스템 + 폴리시)
- `DIRECTION.md` — 장르 믹스 시나리오
- `LORE.md` — 세계관 (신호 무결도, 영 형상 미스터리 등 향후 활용)
- 외부 사례 (이번 세션 검색):
...
2. **탐색**: 1~2 탐색 에이전트로 ctx.\* 호출 위치, shadowBlur 사용처, 사전 렌더되는 sprite cache 패턴 정리.
3. **확인 질문 (사용자 확인 질문)**:
   - 뷰포트: 1280×720 (16:9) vs 1100×700 (현재 유지) vs 1920×1080 (큰 화면 풀)?
   - PIXI 텍스처 사전 렌더 방식: 빌드 타임 캔버스 → PIXI.Texture로 한 번 변환 vs 런타임 PIXI.Graphics?
   - 점진 마이그레이션(Canvas 2D + PIXI 병행 일시) vs 한 번에 전환?
4. **계획 파일 작성**: 각 단계를 독립 커밋 단위로 분해. 핵심 파일 + 라인 명시. 검증 방법 (풀 런 + FPS 측정).
5. **계획 모드 종료**.

작업 중 불확실한 부분(예: PIXI v8 BlurFilter 성능, ParticleContainer 글로우 처리)은 웹 검색로 재검색.
...
- **커밋별 동작 가능** — 각 단계 끝 시점에 게임이 정상 작동해야 함 (점진 마이그레이션).
- **1주차 변경 보존** — 융합 14종 onUpdate, 카드 추가 슬롯, 드랍 규칙, 리롤 초기화, 보스 HP 10배.
